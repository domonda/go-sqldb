...
func (p *OneTimeRowsProvider) QueryRow(query string, args ...interface{}) sqldb.RowScanner {
	p.mtx.Lock()
	defer p.mtx.Unlock()
...
func (p *OneTimeRowsProvider) QueryRows(query string, args ...interface{}) sqldb.RowsScanner {
	p.mtx.Lock()
	defer p.mtx.Unlock()
