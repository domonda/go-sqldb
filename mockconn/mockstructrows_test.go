...
type noColumnsRow struct {
	Unexported int
	Skipped    int `db:"-"`
}

func TestNewMockStructRows_Columns(t *testing.T) {
	rows := NewMockStructRows[simpleRow](nil)
...
func TestNewMockStructRows_EmbeddedStruct(t *testing.T) {
	rows := NewMockStructRows[embeddedRow](nil)
	cols, err := rows.Columns()
	if err != nil {
		t.Fatal(err)
	}
...
func TestNewMockStructRows_ScanSimple(t *testing.T) {
	rows := NewMockStructRows(nil,
		simpleRow{Name: "Alice", Age: 30, Score: 9.5, Active: true},
		simpleRow{Name: "Bob", Age: 25, Score: 7.0, Active: false},
	)
...
func TestNewMockStructRows_ScanTime(t *testing.T) {
	...
	rows := NewMockStructRows(nil, timeRow{ID: 42, CreatedAt: now})
...
func TestNewMockStructRows_ScanValuer(t *testing.T) {
	...
	rows := NewMockStructRows(nil, valuerRow{ID: id, Name: "  hello  "})
...
func TestNewMockStructRows_ScanEmbedded(t *testing.T) {
	rows := NewMockStructRows(nil, embeddedRow{
		embeddedBase: embeddedBase{ID: 1, Name: "test"},
		Extra:        "extra_val",
	})
...
func TestNewMockStructRows_ScanWithoutNext(t *testing.T) {
	rows := NewMockStructRows(nil, simpleRow{Name: "x"})
	var name string
	var age int64
	var score float64
	var active bool
...
func TestNewMockStructRows_PanicNoMappedColumns(t *testing.T) {
	...
	NewMockStructRows[noColumnsRow](nil)
}
