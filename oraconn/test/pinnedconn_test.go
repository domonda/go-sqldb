...
// oraconn: a pinned Connection runs every query on one dedicated backend
// session, identified here by its v$session SID, until Close returns the
// session to the pool.
func TestPinnedConnection(t *testing.T) {
	ctx := t.Context()
	conn := connectOracle(t)

	// The cross-session assertion below needs the pool to hand out a second
	// backend session while the pinned session holds the first, so it requires a
	// pool of at least two connections. Skip rather than block if the pool is
	// capped below that (MaxOpenConnections == 0 means unlimited).
	...
	require.True(t, ok, "oraconn connection must implement sqldb.ConnPinner")

	pinned, err := pinner.Conn(ctx)
	require.NoError(t, err)

	// Every query on the pinned Connection runs on the same backend session.
	sid1, err := sqldb.QueryRowAs[string](ctx, pinned, nil, pinned,
		/*sql*/ `SELECT SYS_CONTEXT('USERENV', 'SID') FROM DUAL`,
	)
	require.NoError(t, err)
	sid2, err := sqldb.QueryRowAs[string](ctx, pinned, nil, pinned,
		/*sql*/ `SELECT SYS_CONTEXT('USERENV', 'SID') FROM DUAL`,
	)
	require.NoError(t, err)
	assert.Equal(t, sid1, sid2, "pinned queries must share one backend session")
	...
	poolSID, err := sqldb.QueryRowAs[string](ctx, conn, nil, conn,
		/*sql*/ `SELECT SYS_CONTEXT('USERENV', 'SID') FROM DUAL`,
	)
	require.NoError(t, err)
	assert.NotEqual(t, sid1, poolSID, "pool query must run on a different session than the pinned one")
...
// pinned oraconn Connection: it is not itself a transaction, Begin starts a
...
func TestPinnedConnectionTransactionAndInfo(t *testing.T) {
	...
	pinnedSID, err := sqldb.QueryRowAs[string](ctx, pinned, nil, pinned,
		/*sql*/ `SELECT SYS_CONTEXT('USERENV', 'SID') FROM DUAL`,
	)
	require.NoError(t, err)
	tx, err := pinned.Begin(ctx, 1, nil)
	require.NoError(t, err)
	txSID, err := sqldb.QueryRowAs[string](ctx, tx, nil, tx,
		/*sql*/ `SELECT SYS_CONTEXT('USERENV', 'SID') FROM DUAL`,
	)
	require.NoError(t, err)
	assert.Equal(t, pinnedSID, txSID, "transaction from a pinned connection must run on the pinned session")
