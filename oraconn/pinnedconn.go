...
// Compile-time check that the oraconn connection implements [sqldb.ConnPinner].
...
// session-scoped state like DBMS_LOCK that must live and die on one session.
...
type pinnedConn struct {
	...
	// The parent non-pinned connection is needed for Config(), Ping(),
	// Stats(), and the lowercaseColumns flag.
	parent *connection
	conn   *sql.Conn
}
...
func (conn *pinnedConn) Query(ctx context.Context, query string, args ...any) sqldb.Rows {
	...
	if conn.parent.lowercaseColumns {
