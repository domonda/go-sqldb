...
func Insert(conn sqldb.Connection, table string, columValues sqldb.Values) error {
	...
	names := make([]string, 0, len(columValues))
	values := make([]interface{}, 0, len(columValues))
	for name, value := range columValues {
		names = append(names, name)
		values = append(values, value)
	}
	// TODO sort by name to create reproducable queries
	return insert(conn, table, names, values)
}

func insert(conn sqldb.Connection, table string, names []string, values []interface{}) error {
