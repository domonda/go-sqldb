...
// Information offers database catalog
// introspection methods: enumerate tables, views, columns, and stored routines
// in the connected database, and check whether a specific object exists.
//
// # Conventions
//
// Listing methods (Tables, Views, Routines) take an optional variadic
// schema filter. With no schema arguments, the result spans every schema
// the connected user can see. With one or more schema arguments, the
// result is restricted to those schemas. The exact meaning of "schema"
// depends on the vendor: a PostgreSQL/MSSQL schema, a MySQL/MariaDB
// database, or an Oracle owner. Returned names are schema-qualified as
// `schema.name` so the result is unambiguous regardless of filter.
//
// Existence methods (TableExists, ViewExists, ColumnExists,
// RoutineExists) accept either a fully qualified identifier or an
// unqualified name. Unqualified names resolve against the database's
// default schema: PostgreSQL search_path, MySQL/MariaDB current
// database, SQL Server default schema, Oracle current user. See each
// method's godoc for any per-method nuance (e.g. RoutineExists has two
// matching modes selected by the presence of parentheses).
//
// # Tables and views are kept separate
//
// Tables and views are distinct kinds. TableExists and Tables match
// only base tables; ViewExists and Views match only views. The
// metadata methods that take a relation argument fall into two
// groups based on what the SQL standard considers meaningful for that
// relation kind:
//
//   - Methods that target tables only — PrimaryKey, ForeignKeys —
//     return a wrapped sql.ErrNoRows when the named relation does not
//     exist OR exists but is a view (views have no primary key or
//     foreign key constraints).
//   - Methods that target tables and views — Columns, ColumnExists —
//     accept either kind and return a wrapped sql.ErrNoRows only when
//     no relation of either kind matches the name.
//
// Routine methods (Routines, RoutineExists) are independent of the
// table/view split and return their natural empty-or-false answer
// without sql.ErrNoRows; "routine does not exist" is exactly what
// `false` from RoutineExists means.
//
// # Vendor support
//
// Implementations exist for vendors that expose either ISO
// `information_schema` views (PostgreSQL, MySQL, MariaDB, SQL Server)
// or an equivalent vendor-specific catalog (Oracle data-dictionary
// views, SQLite `sqlite_schema` + PRAGMA). Not every method needs to be
// supported by every driver; methods that cannot be implemented on a
// given vendor return `errors.ErrUnsupported`.
//
// # Relationship to the information package
//
// The [github.com/domonda/go-sqldb/information] subpackage offers
// lower-level helpers that work directly against `information_schema`
// (typed structs for catalog rows, helper queries for primary keys and
// foreign keys, an HTML inspector). Information is the
// higher-level, vendor-portable surface; drivers implement it on top of
// either the information package or a vendor-specific catalog.
type Information interface {
	...
	// to resolve unqualified identifiers in this interface's
	...
	Views(ctx context.Context, schema ...string) ([]string, error)
	...
	ViewExists(ctx context.Context, view string) (bool, error)
	...
	Columns(ctx context.Context, tableOrView string) ([]ColumnInfo, error)
	...
	// (false, nil).
	ColumnExists(ctx context.Context, tableOrView, column string) (bool, error)
	...
	// primary key); use [Information.Columns] if you need the columns
	// of a view.
	...
	//
	// Each [ForeignKeyInfo] entry describes one constraint. Composite
	...
	Routines(ctx context.Context, schema ...string) ([]string, error)
	...
	RoutineExists(ctx context.Context, routine string) (bool, error)
}
