...
- **Pinned Connections**: `sqldb.ConnPinner` / `sqldb.PinConn` check out one dedicated session and pin it for the lifetime of the returned `PinnedConnection`, required for session-scoped state like PostgreSQL `pg_advisory_lock`, `SET SESSION` settings, and temporary tables that a pooled `*sql.DB` cannot keep on one backend. The `db` package wraps this as the `db.PinnedConn` / `db.PinnedConnResult` callback helpers (`db/pinnedconn.go`). Implemented by `pqconn`, `mysqlconn`, `mssqlconn`, and `oraconn`; `sqliteconn` has no pool so it returns `errors.ErrUnsupported`. A transaction is already bound to one session, so `PinConn` on a transaction returns `ErrWithinTransaction`. See README "Pinned connections (session-scoped state)".
...
- **Argument Redaction**: `sqldb.Secret` / `sqldb.KeepSecret` redact individual args in formatted queries (used by `FormatValue` so error messages built via `WrapErrorWithQuery` never leak secrets). `sqldb.ConnectionWithoutPlaceholderSubstitution` wraps a `Connection` so placeholders stay literal connection-wide, including across `Begin`. `sqldb.ConnectionWithoutQueryInErrors` goes further and keeps the query itself out of errors by implementing the `sqldb.QueryInErrorsOmitter` interface. `sqldb.WrapErrorWithQuery` always wraps as its name says; the query functions call `sqldb.WrapErrorWithQueryIfConfigured` instead, which is the only place that consults `QueryInErrorsOmitter`. `sqldb.UnwrapErrorWithQuery` (mirrored as `db.UnwrapErrorWithQuery`) strips the query from a single error, finding it via `errors.AsType` so it also works through standard `%w` wrapping. See README "Redacting secret arguments in logs and errors". Any custom `QueryFormatter` implementation must provide `SubstitutePlaceholders(query, args) (string, error)`.
...
- Requires Go 1.26: every module `go.mod` and `go.work` declare `go 1.26.0`
