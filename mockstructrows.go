...
var _ Rows = new(MockStructRows[struct {
	ID int `db:"id"`
}])

// NewMockStructRows returns a new MockStructRows with column names
// derived from the struct fields of S using the given reflector
// (or NewTaggedStructReflector() if nil) and the given rows as data.
// Panics if S is not a struct or has no mapped columns.
func NewMockStructRows[S any](reflector StructReflector, rows ...S) *MockStructRows[S] {
	t := reflect.TypeFor[S]()
	if t.Kind() != reflect.Struct {
		panic(fmt.Sprintf("sqldb.NewMockStructRows: type parameter S must be a struct, got %s", t))
	}
	if reflector == nil {
	...
	if len(columnInfos) == 0 {
		panic(fmt.Sprintf("sqldb.NewMockStructRows: struct %s has no mapped columns", t))
	}
